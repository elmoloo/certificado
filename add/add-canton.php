...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre_cant==""){
		...
		$insert	=	$db->insert('canton',$data);
		if($insert){
			header('location:add-canton.php?msg=ras');
			exit;
		}else{
			header('location:add-canton.php?msg=rna');
			exit;
		}
	}
}
...
<?php include_once('navbar.php');?>
				...
				<div class="col-sm-6">
					...
					<form method="post">
						...
						<div class="form-group">							
							...
							<select id="id_provincia"name="id_provincia"class="form-control"  required>
								...
								<?php foreach ($provincia as $pro){ ?>								
									<option  value="<?php echo $pro->id_provincia?>"><?php echo $pro->nombre_prov?></option>
								<?php } ?>
							</select>						
						...
						<div class="form-group">
							...
							<button type="submit" name="submit" value="submit" id="submit" class="btn btn-primary"><i class="fa fa-fw fa-plus-circle"></i> Añadir Canton</button>
	...
	<?php
	$condition	=	'';
	if(isset($_REQUEST['nombre_cant']) and $_REQUEST['nombre_cant']!=""){
		$condition	.=	' AND nombre_cant LIKE "%'.$_REQUEST['nombre_cant'].'%" ';
	}
	
	if(isset($_REQUEST['id_provincia']) and $_REQUEST['id_provincia']!=""){
		$condition	.=	' AND id_provincia LIKE "%'.$_REQUEST['id_provincia'].'%" ';
	}
		...
		<div>
			<table class="table table-striped table-bordered">
				...
				<tbody>
					...
					<tr>
						<td><?php echo $s;?></td>
						<td><?php echo $val['nombre_cant'];?></td>
                        <td>
						<?php 
							
							$consulta_provincua=	'';
							$consulta_provincia = $pdo->query("SELECT * FROM  provincia where id_provincia =".$val['id_provincia']);	
							$provincia= $consulta_provincia->fetchAll(PDO::FETCH_OBJ);
							foreach ($provincia as $pai){
								echo $pai->nombre_prov;
							}
						...
						<td align="center">
							<a href="../edit/edit-canton.php?editId=<?php echo $val['id_canton'];?>" class="text-primary"><i class="fa fa-fw fa-edit"></i> Editar</a> | 
							<a href="../delete/delete-canton.php?delId=<?php echo $val['id_canton'];?>" class="text-danger" onClick="return confirm('Are you sure to delete this user?');"><i class="fa fa-fw fa-trash"></i> Eliminar</a>
						</td>
